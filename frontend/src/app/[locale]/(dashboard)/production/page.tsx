      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 'var(--space-4)' }}>
        ...
        <Card style={{ padding: 'var(--space-4)', display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 'var(--text-xl)', fontWeight: 'var(--font-bold)', color: 'var(--color-text-primary)' }}>
              {formatCurrency(totalCost, locale, defaultCurrency)}
            </div>
          </div>
        </Card>
      </div>
          ...
          <tbody>
            {orders.map((ord, idx) => (
              <tr key={ord.id} style={{ borderBottom: idx === orders.length - 1 ? 'none' : '1px solid var(--color-border-light)' }}>
                ...
                <td style={{ padding: '12px 16px', fontWeight: 'var(--font-medium)' }}>{formatCurrency(ord.estimatedCost, locale, (ord as any).currency || defaultCurrency)}</td>
